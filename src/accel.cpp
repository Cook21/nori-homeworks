...
void Accel::build(const Point3f& cameraPos)
{
    this->cameraPos = cameraPos;
    std::function<OctTreeNode*(TBoundingBox<Point3f>*, std::vector<Triangle>*, const std::vector<Mesh*>&, int depth)> buildTree;
    buildTree = [&buildTree, &cameraPos](TBoundingBox<Point3f>* box, std::vector<Triangle>* triangles, const std::vector<Mesh*>& meshes, int depth) -> OctTreeNode* {
...
bool Accel::rayIntersect(const Ray3f& ray_, Intersection& its, bool shadowRay) const
{
    ...
    std::function<std::pair<bool, Triangle>(OctTreeNode& node, Ray3f& ray, Intersection& its, const std::vector<nori::Mesh*>& meshes)> octTreeSearch;
    octTreeSearch = [&octTreeSearch, this](OctTreeNode& node, Ray3f& ray, Intersection& its, const std::vector<nori::Mesh*>& meshes) -> std::pair<bool, Triangle> {
        bool didFound = false;
        Triangle itsTriangle = { 0, 0 };
        if (node.boundingBox->rayIntersect(ray)) {
            if (node.isLeaf()) {
                ...
                if (ray.o == this->cameraPos) {
                    sortedChildNodes = node.childs;
                } else {
                    sortedChildNodes = node.getSortedChilds(ray.o);
                }
                for (int i = 0; i < 8; i++) {
                    if (sortedChildNodes[i] != nullptr) {
                        if (didFound && sortedChildNodes[i]->boundingBox->distanceTo(ray.o) > its.t)
    ...
    */
    ...
    didFoundIntersection = result.first;
    itsTriangle = result.second;
    if (didFoundIntersection && !shadowRay) {
